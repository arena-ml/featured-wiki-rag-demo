...
mv vectorstore_index.faiss "${INPUT_DIR}"/input2/output1/llm1-rag/

#temp way to indicate this data is from step 2 of pipeline.
mv WikiRC_StepOne.json WikiRC_StepTwo.json

mv WikiRC_StepTwo.json  "${INPUT_DIR}"/input2/output1/llm1-rag/

find "${INPUT_DIR}"/input2/output1/ -mindepth 1 -maxdepth 1 ! -name 'embed-data' -exec mv {} "${OUTPUT_DIR}"/output2/ \;
