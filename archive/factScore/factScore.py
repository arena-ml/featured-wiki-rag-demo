...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("factscore_evaluation.log")],
)
...
class FactScoreEvaluator:
    ...
    def initialize_embeddings(self) -> langchain_ollama.OllamaEmbeddings:
        ...
        try:
            embed_model = langchain_ollama.OllamaEmbeddings(
    ...
    def load_articles(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(articles)} articles from {self.config['inputFile']}"
            )
            return articles
    ...
    def compute_factscore(
        ...
        try:
            for question, gen_answer in generated_answers.items():
                ...
                self.console.print(
                    f"[yellow]Cosine Similarity:[/yellow] {similarity:.4f}"
                )
    ...
    def save_results(self, output_file: Optional[str] = None) -> None:
        ...
        try:
            output_path = output_file or self.config.get(
                "outputFile", "factscore_results.json"
            )
...
def main():
    ...
    try:
        # Configuration
        config = {"inputFile": "WikiRC_ESO.json", "outputFile": "factScore.json"}
